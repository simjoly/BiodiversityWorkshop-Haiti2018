---
title: "Atelier sur les données de biodiversité"
author: "Simon Joly"
...
Le but de cet atelier est de donner une introduction à certains types d’analyses qui peuvent être effectuées à l’aide de données d’occurrences de biodiversité. Il contient des exercises dans le logiciel R. Bien que le logiciel soit un peu complexe d’utilisation, les avantages à l’utiliser compensent largement. En effet, une fois que les rudiments de R sont maîtrisés, il est possible d’effectuer une panoplie d’analyses, et même d’en développer selon ses propres besoins.
...
### Carte montrant plusieurs espèces

Il pourrait être intéressant de montrer les différentes espèces, par exemple en donnant une couleur différente à chaque espèce. Une telle carte demande un peu plus d'étapes à faire, mais ce n'est pas très compliqué.
...
Comme vous pouvez voir, il y a un total de 40 espèces de *Miconia* pour lesquelles on a des données d'occurrence à Haïti.

Maintenant, on veut créer un vecteur de couleur pour associer chaque espèce à une couleur différente. Ceci peut être fait avec la fonction.

**Cette section n'est pas complétée...**
