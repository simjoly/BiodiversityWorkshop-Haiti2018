...
```{r "Plot Caribbean map", fig.height=4, fig.width=5, fig.align='center'}
# Charger une carte des Caraïbes
plot(caribbean)
```
...
```{r "Nicer map", fig.height=4, fig.width=5, fig.align='center'}
...
```{r "Nicer map with points", fig.height=4, fig.width=5, fig.align='center'}
...
Ces variables environementales peuvent aussi servir à faire des modèles de distribution d'espèces.

Pour pouvoir faire un modèle de distribution des espèces, on doit avoir des variables disponible pour l'ensemble du territoire où se trouvent les occurences et ou on veut prédire la probabilité de présence des espèces. On veut idéalement le plus de variable environnementales possible, mais on commence souvent à l'aide de données bioclimatiques, c'est à dire qui utilisent la température et les précipitation, parce qu'elles sont très facile à obtenir. La source la plus communément utilisée est [WorldClim](http://www.worldclim.org/version1), où il est possible d'obtenir les données climatiques actuelles, mais aussi passées et futures. Il suffit de les télécharger, de les décrompresser, et de placer le dossier dans le répertoire de travail.


On peut maintenant se lancer dans la modélisatin de distribution d'espèces. L'idée avec cette approche est de se demander où les espèces sont le plus propices de se retrouvé étant donné les conditions dans lesquelles elles ont été observées.

Plusieurs méthodes existent pour de telles modélisation, mais plusieurs requièrent des données d'absence, c'est à dire des endroit où l'on est certain que l'espèce ne se trouve pas. C'est un type de données qui n'est généralement pas disponible avec les données d'occurence. Certaines méthodes acceptent cependant des pseudo-absences, c'est à dire des points aléatoirement pigés dans la région que l'on veut modéliser. C'est le cas de la méthode appellée MaxEnt. Pour utiliser MaxEnt, il faut la télécharger d'internet et ensuite la placer dans la package `dismo` pour qu'il puisse être utilisé.

Par faciliter l'installation, j'ai placé une copie de MaxEnt (maxent.jar) dans le dossier de l'atelier. Il faut siplement le copier dans le logiciel `dismo`. Pour ce faire, exécutez le bloc de commande suivant dans R.
...
La première étape consiste donc à sélectionner des points aléatoire dans la région d'intérêt.
...
Si des données environnementales existent pour d'autre période de temps, il est possible de prédire la distribution des espèces dans ces nouveaux cadre. Les résultats sont à interpréter avec prudence, parce que cela implique que rien d'autre que les variables étudiées vont affecter les distributions d'espèces. Hors, les espèces peuvent migrer s'adapter à des nouvelles conditions et d'autres variables non étudiées pourraient s'avérer très importantes pour prédire la distribution des espèces. Néanmoins, ça peut être un exercise intéressant.

Des modèles climatiques ont permis de prédire la tepérature et les précipitations dans le futur. De telles données sont disponibles sur le ste de worldclim et peuvent être téléchargées afin de tester où il serait le plus probable de trouver les espèces dans le futur.

Pour ce faire, il s'agit seulement d'utiliser le modèle contruit avec les données actuelles pour prédire la présence de l'espèces sur les données futures. Les données futures ont été chargées plus tôt avec les données actuelles.
